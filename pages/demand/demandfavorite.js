...
Page({
  ...
  onPullDownRefresh: function () {
    ...
    this.getDemandFavorite(function () {
  ...
  getDemandFavorite: function (callback) {
    var that = this;
    var demandService = new DemandService();
    demandService.getDemandFavorite(this.data.pageNum || 1, function (demandList) {
  ...
  unstarDemand: function (e) {
    ...
    demandService.starDemand(id, 3, function (result) {
      that.data.demands.splice(e.currentTarget.dataset.index, 1)
      wx.showToast({
        title: '取消成功'
      })
      that.setData({
